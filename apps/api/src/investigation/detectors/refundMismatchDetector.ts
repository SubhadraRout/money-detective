...
export function detectRefundAmountMismatch(
  ...
  for (const refund of dataset.refunds) {
    ...
    const difference = roundMoney(
      refund.amount - refund.expectedAmount
    );
    ...
    const payment = dataset.payments.find(
      (item) =>
        item.paymentId === refund.paymentId
    );
    ...
    candidates.push({
      ...
      evidence: [
        ...
        refundEvidence(
          refund,
          "expectedAmount",
          `Expected refund amount was ${refund.expectedAmount}.`
        ),

        refundEvidence(
          refund,
          "amount",
          `Actual processed refund amount was ${refund.amount}.`
        ),
      ],

      deterministicReason:
        `Expected refund amount was ${refund.expectedAmount}, but the processed refund amount was ${refund.amount}. Difference: ${Math.abs(difference)}.`,
    });
  }

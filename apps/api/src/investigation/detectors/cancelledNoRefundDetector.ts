      ...
      candidates.push({
        ...
        evidence: [
          ...
          paymentEvidence(
            payment,
            "status",
            "Payment was captured for a cancelled order."
          ),
          ...
          ...processedRefunds.map(
            (refund) =>
              refundEvidence(
                refund,
                "amount",
                `Processed refund amount was ${refund.amount}.`
              )
          ),
        ],

        deterministicReason:
          `Cancelled order ${order.orderId} has a captured payment of ${payment.amount}, but only ${totalRefunded} has been refunded. Remaining financial exposure is ${remainingExposure}.`,
      });
    }
  }

...
export function detectMissingSettlements(
  ...
  const settledPaymentIds = new Set(
    dataset.settlements.map(
      (settlement) => settlement.paymentId
    )
  );
    ...
    candidates.push({
      ...
      evidence: [
        paymentEvidence(
          payment,
          "status",
          "Payment is captured but no settlement record exists."
        ),
        paymentEvidence(
          payment,
          "amount",
      ...
      deterministicReason:
        `Captured payment ${payment.paymentId} has no corresponding settlement.`,
    });
  }

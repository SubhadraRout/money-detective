...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const report =
        await investigateCaseWithOllama(
          investigationCase as Parameters<
            typeof investigateCaseWithOllama
          >[0],
          evidence as unknown as Parameters<
  typeof investigateCaseWithOllama
>[1]
        );
      ...
      res.status(502).json({
        error:
          "AI investigation failed. Make sure Ollama is running with qwen3:4b.",
      });
    }
  }
);

function getActionType(investigationCase) {
...
function getRecoverySteps(investigationCase) {
    switch (investigationCase.type) {
        case "DUPLICATE_REFUND":
            return [
...
function getHumanReviewReason(investigationCase) {
...
function getRationale(investigationCase, aiReport) {
    return (`${investigationCase.title}. ` +
...
export function createRecoveryPlan(investigationCase, aiReport) {
    const actionType = getActionType(investigationCase);
    const steps = getRecoverySteps(investigationCase);
    const humanReviewReason = getHumanReviewReason(investigationCase);
    const potentialRecovery = investigationCase.financialImpact?.potentialLeakage;
    return {
        caseId: investigationCase.caseId,
        recoveryStatus: "human_review_required",
        recoverability: investigationCase.recoverability,
        recoverabilityReason: investigationCase.recoverabilityReason,
        financialImpact: {
            potentialRecovery,
            currency: "INR",
        },
        recoveryAction: {
            type: actionType,
            action: investigationCase.recommendedAction.action,
            owner: investigationCase.recommendedAction.owner,
            priority: investigationCase.recommendedAction.priority,
        },
        rationale: getRationale(investigationCase, aiReport),
        steps,
        humanReview: {
            required: true,
        ...
        verification: {
            criteria: investigationCase.verification.criteria,
            expectedOutcome: investigationCase.verification.expectedOutcome,
        },
        aiContext: {
            confidence: normalizeConfidence(aiReport.finding.confidence === "high"
                ? 1
                : aiReport.finding.confidence === "medium"
                    ? 0.7
                    : 0.4),
            finding: aiReport.finding.merchantExplanation,
            recommendedNextStep: aiReport.finding.recommendedNextStep,
        },
    };
}
export function createRecoveryPlans(investigationCases, aiReports) {
    const reportsByCaseId = new Map(aiReports.map((report) => [
        report.caseId,
        report,
    ]));
    return investigationCases
        .map((investigationCase) => {
        const aiReport = reportsByCaseId.get(investigationCase.caseId);
        if (!aiReport) {
            return null;
        }
        return createRecoveryPlan(investigationCase, aiReport);
    })
